...
複数の `.sock` が並んでいる状態で `--socket` / `CHROME_MCP_SOCKET` を指定せず起動した場合は、最初のツール呼び出しでエラーを返し「`browser_list_profiles` を使って候補を確認し、ユーザーにどちらに繋ぐか確認してから `browser_select_profile` を呼べ」と LLM に促します。勝手にどちらかへ接続して操作することはしません。
